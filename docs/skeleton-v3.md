...
GET  /admin/events?date=&target_type=&limit=&offset= → [{id, actor, target_type, target_id, action, from_state, to_state, detail, created_at}]   # M-08d ③ 감사 로그. admin_events + risk_report_events 읽기 시 병합(저장 이중화 없음, M-36) — report 행은 target_type='report'·target_id=report_id·detail=note|null. id = 원본 테이블 id(report 행은 risk_report_events.id), 유일키 = target_type+id. date=Asia/Seoul 일자, 정렬 created_at desc(최신 우선, 병합으로 id 정렬 폐지), limit 기본 50·최대 200, offset 기본 0. 읽기 전용
