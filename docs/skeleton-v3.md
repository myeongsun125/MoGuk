...
| M-35 | activate 응답 lang (2026-08-31, 명선 확정 / 리허설 D-2)<br>결정: ① POST /auth/activate 응답에 lang(workers.lang, vi\|in) 추가 — §3 비파괴 확장 ② 프론트는 응답 lang으로 기본 언어 초기화, 근로자 변경 토글 유지 ③ 실패 응답은 사유 불문 401 유지(토큰 존재 비노출), 프론트 안내 3종(토큰 파라미터 없음·401·완료) ④ JWT claim·M-28b identity 무접촉<br>R6: 리허설 0831 — 프론트 lang이 UI 기본값(vi)이라 in 워커 화면·요청이 vi로 나감(q66~69 lang=ko, 보고 #3 lang=vi). 영향: SB(응답), JH(초기화·안내) ⑤ 이월(2026-09-01): 활성화 후 언어 변경 경로(홈·탭 토글)는 본선 후(#61 회부 판정 A) | 확정 (2026-08-31, 명선) |
