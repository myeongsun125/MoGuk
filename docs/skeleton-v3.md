...
근로자:
```
POST /auth/activate        {token, pin}                    → {jwt, refresh, lang}   # lang = workers.lang(vi|in) 반환 — 프론트 기본 언어, 근로자 변경 토글 허용 (M-35 비파괴 확장). 실패는 사유 불문 401 유지
...
GET  /learn/cards?module=safety|learning&lang= → {module, quiz_set_id, cards[{id, kind:'phrase'|'term', text, text_ko, high_risk, note_ko?, src?}]}   # safety=phrases 시드(text_vi/in), learning=glossary(term_vi/in), lang 해석 퀴즈 동형(vi/in 지정 → 근로자 lang → ko), module 미지정·그 외 422, quiz_set_id는 module 세트 부재 시 null, LLM 호출 없음 (M-42)   # optional_identity(미인증 200·ko 폴백), high_risk=phrase 시드값·term false, glossary approved∪draft
...
POST /reports/{id}/confirm {result:'confirmed'|'corrected', corrected_text?} → {id, result, reporter_confirmed} | corrected → {id, result, requeued_job_id, reporter_confirmed:false} | 요약 실패건 → {id, result:'local_failed', original_text, message}   # M-08c 비차단(접수·관리자 노출을 막지 않음). 인증 필수 — 미인증 401(M-37). actor='worker:<wid>' — M-28b identity 소비. 신원 필드 본문 수신 금지(400)
...
POST /admin/unanswered/{id}/answer {text} → {id, status:'answered', answered_at, ingest_job_id}   # → ingest_answer job → documents(origin='admin_answer') 편입 (M-05). open 에서만(그 외 422·대상 없음 404). 전이마다 admin_events 1행(M-08d, actor는 M-15b). chunks.meta category 'general'·draft false — 즉시 검색 노출
...
| M-10a | 게이트 임계값 τ | **미결 → 8/29 gate_eval 실측 확정** → 갱신(2026-08-31): 가값 GATE_TAU env 0.45 운영(0.80은 교차언어 코사인으로 정상 답변 전건 차단, M-34 ③), 정식 τ = gate_eval 스윕(정상 30·오염 60) 후 env만 조정 |
...
| M-23 | 암호문 시연 저장소 = edge_net 의 경량 edge-db(`published_content` 단일 테이블). 폴백 = 봉인 컬럼 at-rest. compose 에 `edge-db` 자리만(profile full) | **8/26 알파에서 확정** → 표기 정형화(2026-09-01): 확정(2026-08-26, 명선 — 알파) |
...
| M-32 | 근로자 초대 발급 (2026-08-31, 명선 확정 / SB 상신·구현 #51·#53)<br>결정: ① POST /admin/workers/invite — 만료 72h·emp_no당 미사용 1건(만료 후 신규)·미활성 재초대 허용(invited_at 매 발급 갱신, 이력은 admin_events)·활성 409 ② admin_events worker_invited(actor M-15b) ③ invite_url = BASE_URL env 접두(값 BG) ④ 토큰 token_urlsafe(32) 원값(소비측 auth 원값 조회 정합) ⑤ 검증: emp_no·name 빈값·lang CHECK 외 422, M-08b ④ 신원 필드 가드 400 동일 적용(필드 누락·타입 오류 동시 시 pydantic 422 선행, 빈값·lang 범위 오류 동시 시 400 선행) ⑥ M-32a 만료 임박 표시(JH) ⑦ M-32b edge 경로 M-28a 릴레이 경유(M-28c 동형 — /admin/* 스텁은 M-28c 범위 외였음, 본 결정으로 확장)<br>R6: 리허설 진입 토큰 발급 경로 부재(invite 스텁). 영향: SB(구현), JH(M-32a·소비), BG(BASE_URL env) | 확정 (2026-08-31, 명선) |
| M-33 | jobs 러너 기동 (2026-08-31, 명선 확정 / SB 상신·구현 #50)<br>결정: ① core-api lifespan에서 run_jobs를 to_thread로 기동(poller_enabled 분기 안, core 전용) ② stop 신호 threading.Event, 종료 순서 set→cancel→await — relay poller 패턴 복제(이견 (a) "set 후 await" 문면은 실구현으로 대체) ③ JOBS_POLL_S env 기본 2.0 ④ compose 무접촉<br>R6: 러너 미기동으로 V3-1 DoD(접수→ko_summary) 미실측. 영향: SB | 확정 (2026-08-31, 명선) |
| M-34 | τ 게이트 경량 c = V4-1 (2026-08-31, 명선 확정 / SB 상신·구현 PR-5)<br>결정: ① verify_backtranslation 실구현 — 되번역 local complete() 1회 + bge-m3 코사인 ② src 게이트 기본 = 원질문(BLUEPRINT §4-1), 근거 청크 결합문 score는 trace 병기, 게이트 src env 선택(기본 question) ③ τ env 가값 0.80 — 실측 후 env만 조정(M-30 ④ 동형), 상수 금지 ④ 되번역 timeout env 가값 10s + M-30 ② 잔여 규칙 min(env, LLM_DEADLINE_S−경과), 되번역 미수행·실패 시(타임아웃 포함) fail-open(passed=true·score=null·gate_reason=null·trace 타임아웃 표기) ⑤ is_high_risk = 검색 청크 category=='safety'(classify 스텁 → OR 항 유보), gated = grounded=false OR (high_risk AND passed=false), 비안전 τ 미달은 배지 ⑥ gate_reason M-10b 3값 — grounded=false 전 경로 "grounding", τ 차단 "threshold" ⑦ trace.verify 실측 기록(되번역문·src 종류·두 score·판정·타임아웃·소요 ms) ⑧ 검출률·오탐률·τ 실측은 리허설 후 별건(APPEAL C3, REHEARSAL:17)<br>R6: M-10a τ 실측 미실시(tenant_settings τ 키 부재), V4-1 미구현 상태로 리허설 진입 불가. 영향: SB(구현), JH(types 소비 선택), BG(env 3종) | 확정 (2026-08-31, 명선) |
| M-35 | activate 응답 lang (2026-08-31, 명선 확정 / 리허설 D-2)<br>결정: ① POST /auth/activate 응답에 lang(workers.lang, vi\|in) 추가 — §3 비파괴 확장 ② 프론트는 응답 lang으로 기본 언어 초기화, 근로자 변경 토글 유지 ③ 실패 응답은 사유 불문 401 유지(토큰 존재 비노출), 프론트 안내 3종(토큰 파라미터 없음·401·완료) ④ JWT claim·M-28b identity 무접촉<br>R6: 리허설 0831 — 프론트 lang이 UI 기본값(vi)이라 in 워커 화면·요청이 vi로 나감(q66~69 lang=ko, 보고 #3 lang=vi). 영향: SB(응답), JH(초기화·안내) | 확정 (2026-08-31, 명선) |
| M-36 | 감사 로그 병합 조회 (2026-08-31, 명선 확정 / 리허설 D-6)<br>결정: ① GET /admin/events가 admin_events와 risk_report_events를 읽기 시 병합 — 저장 이중화 없음 ② report 행 매핑 target_type='report'·target_id=report_id·detail=note\|null, action은 risk_report_events 원값, id는 원본 테이블 id(유일키 = target_type+id) ③ 정렬 created_at desc, date·limit·offset·target_type 필터 병합 결과에 동일 적용 — §3 /admin/events 행 갱신은 구현 PR(Q5)과 동일 커밋(3중 대조 테스트 test_admin_approval.py:648) ④ 기존 admin_events 단정·M-08d 기록 규약 무변경<br>R6: 리허설 ⑩ — 위험보고 ack·resolve·original_viewed가 상세 이력에만 있고 글로벌 감사 로그에 없음(어필 ①② 가시성). 영향: SB(조회), JH(화면 표시) | 확정 (2026-08-31, 명선) |
| M-37 | confirm 인증 강제·타임스탬프 정합 (2026-09-01, 명선 확정 / 리허설 D-4·D-8, SB 구현 Q7)<br>결정: ① POST /reports/{id}/confirm optional_identity → require_worker — 헤더 부재·위조·만료 전부 401(사유 무구분), actor는 검증된 claims wid에서만 ② 릴레이 경로 동일 차단 — dispatch가 confirm() 직접 호출, identity None이면 core 401, 릴레이 응답 계약(M-28a·M-28b·M-08b ④) 무변경 ③ POST /reports 접수·조회는 optional_identity 유지(인증 강제 전면 도입은 기존 이월), M-08c 비차단 의미 유지 ④ 타임스탬프 정합(D-8): risk_reports.processed_at·summary_done 이벤트 created_at = clock_timestamp() — 트랜잭션 now()의 claim 시각 오기록 해소, 실패 경로 NULL 유지<br>R6: 리허설 D-4 — 미인증 confirm 200 + actor 'worker:unauthenticated'(#2는 worker:5 — 요청 Bearer 부재와 optional 허용 중첩) / D-8 — 요약 소요가 픽업 시각으로 기록. 영향: SB(구현·test:760 반전), JH(토큰 상시 첨부 Q4·401 안내) | 확정 (2026-09-01, 명선) |
...
| M-08c | 보고자 확인 루프 — 비차단 (2026-08-28, 명선 확정)<br>결정: ① 접수·관리자 노출은 보고자 확인을 기다리지 않는다(비차단 — 긴급성 우선, V3-1 DoD 5초 submitted 정합). ② 요약 성공 시 보고자 모국어 백트랜슬레이션 확인 요청 병행 — reporter_confirmed/reporter_corrected 이벤트로 감사·lineage 기록, 관리자 요약에 "보고자 확인" 배지. ③ local_failed 시 원문 에코 + 접수 안내. ④ V4 범위 = API·이벤트까지, 확인 UI는 JH V5.<br>R6: 데이터 정확·안전 전달 + 확인의 lineage 기록(명선 제안 08-28). M-10 백트랜슬레이션 게이트와 접점 — τ 실측 시 위험보고 확인률 병행 측정 가능. 채번 정리: 일시 M-05b로 유통 — 위험보고 계열 M-08 하위 재채번(08-29). 영향: SB(구현), JH(V5 UI), MS(측정) | 확정 (2026-08-28, 명선) |
| M-28b | 인증 컨텍스트 릴레이 전파 규약 (2026-08-29, 명선 확정 / SB 상신)<br>결정: ① RelayItem 최상위 identity 필드(dict|None) 추가 — edge가 require_worker 검증 후 claims에서 추출한 wid·tenant만 전파. 원 JWT·Authorization 헤더는 릴레이 경계 미통과, body 혼입 금지(M-08b ④ 정합). ② core는 identity를 서비스 함수 actor/worker_id로 소비, 하드코딩 None 제거. ③ pending 응답 JSON identity 필드 1개 additive 확장 — M-28a 파라미터 5종·큐 메커니즘(long-poll·리스·재배포)·/internal 3종 무접촉. ④ 미인증 경로는 identity=None 기록(보호 전환 시 자연 해소).<br>R6: PR-2 actor의 JWT 실물 소비에 신원 전파 배선 필요(SB 상신 0829 — as_payload 7필드에 신원 부재·worker_id=None 하드코딩 실증). B안(core 역조회 저장소 신설)은 신규 통로·저장소 수반으로 기각. 영향: SB(구현), BG(릴레이 계약 인지) | 확정 (2026-08-29, 명선) |
| M-15b | 관리자 인증 부재 — 전이 actor 임시 처리 (미결, 2026-08-29 SB 상신)<br>현황: V3-2(#24)는 근로자 인증만 실물, 관리자 인증 스텁(POST /auth/admin/login 미구현). PR-2 전이(ack/resolve)의 행위자는 risk_report_events.actor='admin:unauthenticated' 단독 기록. 스냅숏 acked_by·resolved_by(001 정본 integer)는 NULL 유지 — 관리자 인증 도입 시 admin id로 채움. acked_at·resolved_at·resolution_note는 전이 시 갱신. 교체 단일 지점 구현, 신원 필드 본문 수신 금지(400) 유지. 명시 리스크: 관리자 전이의 행위자 식별 불가(NULL·미확인 라벨로 축적). 데모 시연은 단일 관리자 전제라 수용. 관리자 인증 도입 시 본 행 판정·교체. 영향: SB(구현), JH(V3-2 관리 화면 표시) | 미결 (2026-08-29) |
...
| M-04c | 런타임 search_path public 후순위 포함 — M-04b 애플리케이션 잔여분 (2026-08-30, 명선 확정 / SB 상신)<br>결정: tenancy.py 런타임 SET search_path를 {schema}, public 순으로 정정 — M-04b(DDL)와 동일 순서 정합 회복. public은 확장 타입 해석 전용 후순위, M-04 테넌트 격리 유지(M-04b 논거 동일). 회귀 테스트: search_path 설정 문자열 단정 추가(현행 가짜 DB 테스트의 미검출 갭 해소).<br>R6: M-04b가 DDL만 정정, 런타임 미정정 → EC2 실데이터 retrieve 전건 실패(::vector 미해석, SB 특정 0830). 영향: SB(구현), MS(EC2 반영 확인) | 확정 (2026-08-30, 명선 확정) |
