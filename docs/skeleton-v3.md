...
| M-10a | 게이트 임계값 τ — 가값 운영: GATE_TAU env 0.45(2026-08-31 워밍업 실측 — 0.80은 교차언어 코사인 특성으로 정상 답변 전건 차단, M-34 ③ 조정 경위). 정식 τ = gate_eval 스윕(정상 30·오염 60, 유형별 검출률·오탐률) 후 확정, env만 조정(M-30 ④ 동형) | 미결 — 가값 0.45 운영 (2026-08-31, 명선) |
...
| M-20 | 본선 D-day 확정 시 워크플로우 일정 조정 — 본선 9/2–9/3 확정, V6-2(리허설1 0831)·V7 시간표 반영 완료 | 확정 (2026-09-01, 명선 — 일정 반영 종결) |
...
| M-23 | 암호문 시연 저장소 = edge_net 의 경량 edge-db(`published_content` 단일 테이블). 폴백 = 봉인 컬럼 at-rest. compose 에 `edge-db` 자리만(profile full) | 확정 (2026-08-26, 명선 — 알파) |
