...
결정 본문의 원본은 [skeleton-v3.md §8](skeleton-v3.md#8-m-xx-결정-대장) (M-01 ~ M-28). 이 파일은 **비준 이후 추가·변경된 결정의 근거(WORKLOG/추적표) 색인**만 유지한다 — 본문을 여기 중복하지 않는다(R4). 예선 문서 대비 변경 사유(R6)는 [TRACEABILITY.md](TRACEABILITY.md)가 색인한다.
...
| M-25 | 미디어 플로우: edge 암호화 → S3 단기 버퍼(TTL) → core pull → 처리 후 S3 삭제 · IAM 분리(edge write-only / core read+delete) · S3=백업+미디어 버퍼 한정 (2026-08-25 개정, R6 사유 기재) | [TRACEABILITY.md](TRACEABILITY.md) #23 | 확정 |
| M-28 | 실시간 요청 릴레이: edge 큐 적재 → core outbound 폴링 → 처리 → edge 회신 → HTTP 완결 (M-22 준수, 지연은 측정 #4) | WORKORDER §5 초안 → 합의 후 skeleton §3 | 확정 |
