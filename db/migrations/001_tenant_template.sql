-- 001_tenant_template.sql — 테넌트 스키마 템플릿. **DDL 원문의 단일 출처(R4)** — docs/skeleton-v3.md §2 와 충돌 시 이 파일 우선. [명선]
--
-- 적용: scripts/apply_tenant.sh <slug>  ({slug} 치환 후 psql 실행). 멱등 — 재적용 안전 (IF NOT EXISTS / OR REPLACE / ON CONFLICT).
-- 테넌시: schema-per-tenant (M-04). 미들웨어가 요청 컨텍스트에서 SET search_path.
-- 사전 조건: pgvector 확장 (hnsw 인덱스는 0.5.0+). 확장은 DB 레벨 1회.
...
CREATE INDEX IF NOT EXISTS chunks_meta_gin ON chunks USING gin(meta);
...
CREATE OR REPLACE VIEW v_comprehension AS
...
CREATE TABLE IF NOT EXISTS questions (                            -- qa_logs 겸용
  id serial PRIMARY KEY, worker_id int REFERENCES workers(id),
...
CREATE INDEX IF NOT EXISTS questions_trace_gin ON questions USING gin(trace);

CREATE TABLE IF NOT EXISTS unanswered_queue (                     -- M-05
...
CREATE TABLE IF NOT EXISTS conversations (                        -- M-07 정착지원 상담 (로컬 티어 고정)
  id serial PRIMARY KEY, worker_id int REFERENCES workers(id),
  role text NOT NULL CHECK (role IN ('worker','assistant')),
  content text NOT NULL, risk_flag bool NOT NULL DEFAULT false,
  created_at timestamptz DEFAULT now());

CREATE TABLE IF NOT EXISTS notifications (                        -- M-06 알림함(쪽지)
  id serial PRIMARY KEY, worker_id int REFERENCES workers(id),
...
CREATE TABLE IF NOT EXISTS speaking_records (                     -- M-12 (STT 미적용, 녹음만)
  id serial PRIMARY KEY, worker_id int REFERENCES workers(id),
  phrase_id int REFERENCES phrases(id), audio_ref text NOT NULL,
  created_at timestamptz DEFAULT now());

CREATE TABLE IF NOT EXISTS safety_courses (                       -- M-14
  id serial PRIMARY KEY,
...
CREATE TABLE IF NOT EXISTS access_logs (                          -- M-07 감사 로그
