...
import {
  ASK_FIXTURE_GATED,
  ASK_FIXTURE_GATED_THRESHOLD,
  ASK_FIXTURE_GROUNDED,
} from "./fixtures/ask.fixtures";
...
async function mockAsk(req: AskRequest): Promise<AskResponse> {
  await delay(300);
  // "__gated_threshold__"/"__gated__" 포함 질문으로 gate_reason 분기를 재현
  // (로컬 스모크용, 계약 필드 아님).
  if (req.question.includes("__gated_threshold__")) {
    return ASK_FIXTURE_GATED_THRESHOLD;
  }
